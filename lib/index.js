...
function get_iso3166_by_phone(phone) {
  ...
  iso3166_data.every(function(iso3166_datum) {
    ...
    return iso3166_datum.phone_number_lengths.every(function(
      phone_number_length
    ) {
      if (
        phone.match(regex) &&
        phone.length === iso3166_datum.country_code.length + phone_number_length
      ) {
        // it match.. but may have more than one result.
        // e.g. USA and Canada. need to check mobile_begin_with
        return iso3166_datum.mobile_begin_with.every(function(
          mobile_begin_with
        ) {
          if (
            phone.match(
              new RegExp("^" + iso3166_datum.country_code + mobile_begin_with)
            )
          ) {
            iso3166 = iso3166_datum;
            return false;
...
function parse(phone, country) {
  var result = [];

  phone = phone === null || typeof phone !== "string" ? "" : phone.trim();
  country =
    country === null || typeof country !== "string" ? "" : country.trim();
...
module.exports.parsePhoneNumber = parse;
module.exports.validatePhoneNumber = validate_phone_iso3166;
module.exports.getCountryFromPhoneNumber = get_iso3166_by_phone;
module.exports.getCountry = getISO3166;
module.exports.countryDataList = iso3166_data;
